...
function Generate-Code {
    param(
        [string]$businessData,
        [string]$targetSystem = "symmetric-finance-system",
    ...
    $securityCheck = Verify-CodeSecurity -CodeFiles $savedFiles
...
function Deploy-CodeChanges {
    ...
    Write-Host "===================================="
    Write-Host "Deploying code changes to container: $TargetContainer"
    Write-Host "===================================="
...
function Main {
    ...
    switch ($command) {
        "generate" {
            ...
            Generate-Code -businessData $businessData -targetSystem $targetSystem -outputPath $outputPath
        }
        "deploy" {
            $container = ""
            $outputPath = "generated_code"
            ...
            Deploy-CodeChanges -CodeFiles $codeFiles -TargetContainer $container
        }
        "verify" {
            $outputPath = "generated_code"
            ...
            $codeFiles = Get-ChildItem -Path $outputPath -Name "*.py" | ForEach-Object { Join-Path -Path $outputPath -ChildPath $_ }
            $securityCheck = Verify-CodeSecurity -CodeFiles $codeFiles
...
# 导出函数
Export-ModuleMember -Function Generate-Code, Deploy-CodeChanges, Verify-CodeSecurity, Main

# 如果直接运行此脚本
if ($MyInvocation.InvocationName -eq ".\elr-code-generator.ps1") {
    Main $args
}
