...
function Start-Runtime {
    ...
    $containerExe = "elr\elr-container.exe"
    if (Test-Path $containerExe) {
        Write-Host "Starting ELR container..."
        try {
            # Start the container process
            Start-Process -FilePath $containerExe -NoNewWindow -PassThru
...
function Get-Sandbox {
    ...
    Write-Host "Models:            2"
    Write-Host "Resources:         CPU: 25%, Memory: 768MB"
...
# Main function
if ($args.Length -lt 1) {
    Write-Host "Usage: elr.ps1 <command> [options]"
    Write-Host ""
    Write-Host "Commands:"
    Write-Host "  version           Show ELR version"
    Write-Host "  help              Show this help"
    Write-Host "  status            Check ELR status"
    Write-Host "  start             Start ELR runtime"
    Write-Host "  stop              Stop ELR runtime"
    Write-Host "  list              List containers"
    Write-Host "  sandbox           Sandbox management"
    Write-Host ""
    Write-Host "Sandbox subcommands:"
    Write-Host "  sandbox list              List all sandboxes"
    Write-Host "  sandbox get --id <id>     Get sandbox details"
    Write-Host "  sandbox create [--container <container>]  Create sandbox"
    Write-Host "  sandbox start --id <id>   Start sandbox"
    Write-Host "  sandbox stop --id <id>    Stop sandbox"
    Write-Host "  sandbox delete --id <id>  Delete sandbox"
    Write-Host "  sandbox load-model --sandbox-id <id> --model-id <model>  Load model"
    Write-Host "  sandbox unload-model --sandbox-id <id> --model-id <model>  Unload model"
    Write-Host "  sandbox models --id <id>  List models in sandbox"
    Write-Host "  sandbox stats --id <id>   Get sandbox statistics"
    exit 1
}
...
switch ($command) {
    ...
    default {
        ...
        Write-Host "Use 'elr.ps1 help' for usage information"
        exit 1
    }
}
