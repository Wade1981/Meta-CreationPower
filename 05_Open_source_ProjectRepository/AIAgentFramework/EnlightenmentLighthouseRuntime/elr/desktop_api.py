...
"""
ELR Desktop API组件
为ELR-Desktop-Assistant提供专属通信通道
"""
...
class ELRDesktopAPIHandler(http.server.BaseHTTPRequestHandler):
    """ELR Desktop API处理器"""
    ...
    def do_GET(self):
        ...
        if self.path == '/api/desktop/status':
        ...
        elif self.path == '/api/desktop/containers':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            response = [
            ...
            self.wfile.write(json.dumps(response).encode('utf-8'))
        elif self.path == '/api/desktop/files':
            self.list_files()
        elif self.path == '/api/desktop/resources':
            self.get_resources()
        elif self.path == '/api/desktop/health':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            response = {
    ...
    def do_POST(self):
        """处理POST请求"""
        if self.path == '/api/desktop/upload':
    ...
    def do_DELETE(self):
        """处理DELETE请求"""
        if self.path.startswith('/api/desktop/files/'):
            filename = self.path.split('/api/desktop/files/')[1]
...
def start_desktop_api_server(port=8081):
    """启动Desktop API服务器"""
    handler = ELRDesktopAPIHandler
    with socketserver.TCPServer(("", port), handler) as httpd:
        print(f"ELR Desktop API服务器启动在 http://localhost:{port}")
        print("可用端点:")
        print("  GET  /api/desktop/status      - 获取ELR状态")
        print("  GET  /api/desktop/containers  - 获取容器列表")
        print("  POST /api/desktop/upload      - 上传文件")
        print("  GET  /api/desktop/files       - 列出上传的文件")
        print("  DELETE /api/desktop/files/{name} - 删除文件")
        print("  GET  /api/desktop/resources   - 获取系统资源使用情况")
        print("  GET  /api/desktop/health      - 健康检查")
        httpd.serve_forever()

if __name__ == "__main__":
    start_desktop_api_server()
