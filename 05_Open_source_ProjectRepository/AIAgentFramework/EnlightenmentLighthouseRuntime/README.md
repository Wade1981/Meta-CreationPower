...
├── elr/               # 核心运行时
│   ├── container.go   # 容器管理
│   ├── network.go     # 网络管理（包含网络状态查询）
│   ├── runtime.go     # 运行时核心
│   ├── token_manager.go # 令牌管理
│   ├── admin_manager.go # 管理员管理
│   ├── crypto.go      # 加密工具
│   ├── desktop_api.py # 桌面API
│   ├── token_manager.py # Python版令牌管理
│   ├── token_manager.ps1 # PowerShell版令牌管理
│   ├── start_desktop_api.ps1 # 启动桌面API脚本
│   ├── go.mod         # Go模块依赖
│   └── cmd/           # 命令行工具
│       └── main.go    # 主入口
├── cli/               # 命令行工具
...
```bash
# 创建管理员
elr admin create --username admin --role super

# 列出管理员
elr admin list

# 为管理员添加容器权限
elr admin add-permission --username admin --container container-id --manage --read --write

# 从管理员移除容器权限
elr admin remove-permission --username admin --container container-id
```
...
```bash
# 上传文件到容器
elr fs upload --id container-id --local-path local-file --container-path container-path --token admin-token

# 从容器下载文件
elr fs download --id container-id --container-path container-path --local-path local-file --token admin-token
```
...
ELR提供了网络状态查询API，用于获取ELR容器的网络连接信息：

### API 端点

- **GET /api/network/status**：获取网络状态信息
  - 返回与ELR-Desktop-Assistant的专属通信地址和端口
  - 返回与公众接触应用的通信API地址和端口
  - 返回模型API的地址、端口和网络状态
...
```json
{
  ...
  "timestamp": 1773861411
}
```

## 令牌管理 API

ELR提供了完整的令牌管理API，用于管理ELR容器的访问令牌：

### API 端点

- **POST /api/token/create**：创建新令牌
  - 请求体：`{"description": "令牌描述"}`
  - 返回：新生成的令牌
...
- **POST /api/token/refresh**：刷新令牌
  - 请求体：`{"token": "旧令牌", "description": "新令牌描述"}`
  - 返回：新生成的令牌

- **GET /api/token/list**：列出所有令牌
  - 返回：令牌列表和状态
