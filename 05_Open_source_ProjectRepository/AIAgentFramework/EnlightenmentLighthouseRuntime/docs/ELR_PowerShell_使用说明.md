...
| `sandbox stop` | 停止沙箱 |
| `sandbox delete` | 删除沙箱 |
| `sandbox load-model` | 加载模型到沙箱 |
| `sandbox unload-model` | 从沙箱卸载模型 |
| `sandbox models` | 列出沙箱中的模型 |
| `sandbox stats` | 获取沙箱统计信息 |
...
--                 ----            ---------         ------    -------
sandbox-1234567890 sandbox-1       running-container created   2026-03-23 10:00:00
sandbox-0987654321 sandbox-2       test-container    running   2026-03-23 10:01:00
====================================
```
...
```powershell
.\elr.ps1 sandbox get --id <沙箱ID>
```

**示例：**

```powershell
.\elr.ps1 sandbox get --id sandbox-1234567890
```
...
```powershell
.\elr.ps1 sandbox create --container running-container
```
...
```
====================================
Created sandbox: sandbox-1234567890 (sandbox-1)
Container: running-container
Status: created
====================================
```
...
```powershell
.\elr.ps1 sandbox start --id <沙箱ID>
```

**示例：**

```powershell
.\elr.ps1 sandbox start --id sandbox-1234567890
```
...
```
====================================
Started sandbox: sandbox-1234567890 (sandbox-1)
Status: running
====================================
```
...
```powershell
.\elr.ps1 sandbox stop --id <沙箱ID>
```

**示例：**

```powershell
.\elr.ps1 sandbox stop --id sandbox-1234567890
```
...
```
====================================
Stopped sandbox: sandbox-1234567890 (sandbox-1)
Status: stopped
====================================
```
...
```powershell
.\elr.ps1 sandbox delete --id <沙箱ID>
```

**示例：**

```powershell
.\elr.ps1 sandbox delete --id sandbox-1234567890
```
...
```
====================================
Deleted sandbox: sandbox-1234567890
====================================
```
...
```powershell
.\elr.ps1 sandbox load-model --sandbox-id sandbox-1234567890 --model-id model-1
```
...
```
====================================
Loaded model model-1 to sandbox sandbox-1234567890
====================================
```
...
```powershell
.\elr.ps1 sandbox unload-model --sandbox-id sandbox-1234567890 --model-id model-1
```
...
```
====================================
Unloaded model model-1 from sandbox sandbox-1234567890
====================================
```
...
```powershell
.\elr.ps1 sandbox models --id <沙箱ID>
```

**示例：**

```powershell
.\elr.ps1 sandbox models --id sandbox-1234567890
```
...
```
====================================
Models in sandbox sandbox-1234567890:
====================================
ID                 NAME            VERSION     STATUS    PATH
--                 ----            -------     ------    ----
model-1           elr-chat        1.0        loaded    E:\X54\github\Meta-CreationPower\05_Open_source_ProjectRepository\AIAgentFramework\EnlightenmentLighthouseRuntime\micro_model\model\models\elr-chat
model-2           fish-speech     1.0        loaded    E:\X54\github\Meta-CreationPower\05_Open_source_ProjectRepository\AIAgentFramework\EnlightenmentLighthouseRuntime\micro_model\model\models\fish-speech
====================================
```
...
```powershell
.\elr.ps1 sandbox stats --id <沙箱ID>
```

**示例：**

```powershell
.\elr.ps1 sandbox stats --id sandbox-1234567890
```
...
- 新增沙箱管理功能，支持创建、启动、停止、删除沙箱
- 新增 `sandbox` 命令及其子命令：list, get, create, start, stop, delete, load-model, unload-model, models, stats
- 新增容器管理命令：create, run, start-container, stop-container, delete, inspect
- 新增其他命令：run-c, run-python, chat
- 优化版本管理，从Go核心代码读取版本信息
- 完善文档，添加沙箱管理命令的详细说明
